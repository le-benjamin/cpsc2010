...
(define (depth tree)
  (cond
    [(null? tree) 0]
    [(list? tree) (+ 1 (apply max (map depth tree)))]
    [else 0]))
...
(define (sum tree)
  (cond
    [(null? tree) 0]
    [(list? tree) (+ (sum (car tree)) (sum (cdr tree)))]
    [(number? tree) tree]
    [else 0]))
...
(define (prod tree)
  (cond
    ...
    [(number? tree) tree]
    [(list? tree)
     (let ((filtered (filter (lambda (x) (not (equal? x 'none))) (map prod tree))))
       (if (null? filtered)
           'none
           (* (prod (car filtered)) (if (null? (cdr filtered)) 
                                      1 
                                      (prod (cdr filtered))))))]
    [else 'none]))
...
(define (count-if pred tree)
  (cond
    ...
    [else (if (pred tree) 1 0)]))

;;; (count-if odd? '(1 2 3)) 
;;; (count-if even? '(1 2 3)) 
;;; (count-if integer? '(1 (2 (3)))) 
;;; (count-if string? '()) 
;;; (count-if even? '((((((8 8 8))))))) 
;;; (count-if (lambda (x) (> x 5)) '((((((9 9 9))))))) 
...
(define (types tree)
  (cond
    [(null? tree) empty]
    [(list? tree) 
      (let ([types-car (types (car tree))]
           [types-cdr (types (cdr tree))])
       (sort (foldl (lambda (item acc)
                (if (member item acc)
                    acc
                    (cons item acc)))
              types-cdr
    ...
    [else (list (cond
                  ...
                  [else 'other]))])) ; not specified in problem but needed for `cond`
;;; (types '(1 1 1 1)) 
;;; (types '(1 ((a)))) 
;;; (types '(1 (("a"))))
;;; (types '(1 ((#\a))))
...
(define (hextree tree)
  (cond
    [(null? tree) empty]
    [(list? tree) (cons (hextree (car tree)) (hextree (cdr tree)))]
    [else (cond
            [(and (integer? tree) (>= tree 10) (<= tree 15))
              (cadr (assoc tree dectohexalist))]
            [else tree])]))
;;; (hextree '(4 (8 (10 12 14 15 16))))
;;; (hextree '(4 (8 (a c e f 16))))
;;; (hextree '(1 (((2 3 4))))) 
;;; (hextree '(1 (((12 13 14 (15))))))
;;; (hextree '()) 
...
(define (tree-min tree)
  (cond
    [(null? tree) '()]
    [(list? tree) 
      (let ([min-car (tree-min (car tree))]
           [min-cdr (tree-min (cdr tree))])
       (cond
         [(and (equal? min-car '()) (equal? min-cdr '())) '()]
         [(equal? min-car '()) min-cdr]
         [(equal? min-cdr '()) min-car]
         [else (min min-car min-cdr)]))]
    [else tree]))
...
;;; (map-tree even? '(1 (2 (3 (4))))) 
;;; (map-tree (lambda (x) (+ x 1)) '(1 (2 (3 (4 5 6)))))
;;; (map-tree odd? '()) 
