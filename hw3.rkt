...
(provide hours xxxx simulate-lite tm1 config1 config2 ins ins-c-state
         ins-c-symbol ins-n-state ins-n-symbol ins-dir tm-reverse
         i-match? i-lookup conf conf-state conf-ltape conf-symbol
         conf-rtape halted? change-state write-symbol normalize
         shift-head-left shift-head-right next-config tm-parity
         tm-sort )

; Please do not modify the lines above this comment.
; ********************************************************
; CS 201 HW #3  DUE Wednesday, 10/1/2025 at 11:59 pm
;                via gradescope
; ****************************************************************
; Name:
; Email address:
...
; Change blank to special symbol (say 'c)
; Move left until hit first special symbol
; Change first special symbol to its corresponding 0 or 1
; Move right one to get next char to copy
; change char to corresponding special symbol (0 to d and 1 to e)
; switch to corresponding state
; move right until hit a blank
; write 0 or 1 depending on state,
; move left until hit d or e

; q1 move right until blank. change blank to 'c. move left. switch to q2. this is the setup phase and we won't use this state again
; q2 decider stage. change current num to appropriate symbol then change to appropriate state then move right
; q3 0 handler. move until blank then change to 0 then move left. change to state q5
; q4 1 handler. see q3
; q5 move left until special symbol. change special symbol back accordingly, move left and into q2
;    if next special symbol is a blank, enter  q6
; q6 clean up phase, switch 0 and 1 to blank, move left until blank, move right and halt (q7) 
; q7 move 


(define tm-reverse
  (list
    (ins 'q1 0 'q1 0 'R) ; move right over zeroes
    (ins 'q1 1 'q1 1 'R) ; move right over ones
    (ins 'q1 'b 'q2 'c 'L) ; change first blank to 'c, move left to q2

    (ins 'q2 0 'q3 'd 'R) ; change 0 to d, move right to q3
    (ins 'q2 1 'q4 'e 'R) ; change 1 to e, move right to q4
    (ins 'q2 'b 'q6 'b 'R) ; if hit blank, move right to q6

    (ins 'q3 0 'q3 0 'R) ; move right over zeroes
    (ins 'q3 1 'q3 1 'R) ; move right over ones
    (ins 'q3 'c 'q3 'c 'R) ; move right over 'c
    (ins 'q3 'b 'q5 0 'L) ; change first blank to 0, move left to q5

    (ins 'q4 0 'q4 0 'R) ; move right over zeroes
    (ins 'q4 1 'q4 1 'R) ; move right over ones
    (ins 'q4 'c 'q4 'c 'R) ; move right over 'c
    (ins 'q4 'b 'q5 1 'L) ; change first blank to 1, move left to q5

    (ins 'q5 0 'q5 0 'L) ; move left over zeroes
    (ins 'q5 1 'q5 1 'L) ; move left over ones
    (ins 'q5 'c 'q5 'c 'L) ; move right over 'c
    (ins 'q5 'd 'q2 0 'L) ; change 'd to 0, move left to q2
    (ins 'q5 'e 'q2 1 'L) ; change 'e to 1, move left to q2

    (ins 'q6 0 'q6 'b 'R) ; change original zeroes to blanks
    (ins 'q6 1 'q6 'b 'R) ; change original ones to blanks
    (ins 'q6 'c 'q7 'b 'R) ; change 'c back to 'b, move right, and halt (q7)
    ))
...
; a helper function to remove leading 'b's from the left side of a list
(define (normalize-aux tape)
  (cond
    [(empty? tape) '()]
    [(equal? (first tape) 'b) (normalize-aux (rest tape))]
    [else tape]))

(define (normalize config)
  (conf (conf-state config)
        (normalize-aux (conf-ltape config))
        (conf-symbol config)
        (reverse (normalize-aux (reverse (conf-rtape config))))))
...
(define (shift-head-left config)
  (conf (conf-state config)
        ...
        (if (and (empty? (conf-rtape config)) (equal? (conf-symbol config) 'b))
            (conf-rtape config)
...
(define (shift-head-right config)
  (conf (conf-state config)
        (if (and (empty? (conf-ltape config)) (equal? (conf-symbol config) 'b))
            '()
            (append (conf-ltape config) (list (conf-symbol config))))
        (if (empty? (conf-rtape config))
            'b
            (first (conf-rtape config)))
        (if (empty? (conf-rtape config))
            '()
...
(define (next-config mach config)
    (if (halted? mach config)
        ...
        (let* ([inst (i-lookup (conf-state config) (conf-symbol config) mach)]
            [changed (write-symbol (ins-n-symbol inst) (change-state (ins-n-state inst) config))])
                (if (equal? (ins-dir inst) 'L)
                    (shift-head-left changed)
                    (shift-head-right changed)))))
...
;;; (for-each displayln (simulate tm1 (conf 'q1 '() 1 '(1 0 1 0)) 20))
;;; (list
;;;  (conf 'q1 '() 1 '(1 0 1 0))
;;;  (conf 'q1 '(0) 1 '(0 1 0))
;;;  (conf 'q1 '(0 0) 0 '(1 0))
...
; This machine determines the parity of a binary number. It uses two main states: an "even" state (q1)
; and an "odd" state (q2). The machine starts in the even state. As it scans right, it ignores 0s and 
; flips between the even and odd states each time it reads a 1. When it reaches the end of the input 
; (a blank), it writes the final parity result: 0 if it's in the even state, or 1 if it's in the odd 
; state. It then enters a cleanup phase, moving left to erase the original input string before 
; positioning the head to the right of the single-digit output and halting.

(define tm-parity
    (list
        (ins 'q1 0 'q1 0 'R) ; move over zeros
        (ins 'q1 1 'q2 1 'R) ; switch to 'q2 when a one is read, continue moving right

        (ins 'q2 0 'q2 0 'R) ; move over zeros
        (ins 'q2 1 'q1 1 'R) ; switch to 'q1 when a one is read, continue moving right

        (ins 'q1 'b 'q3 '0 'L) ; write 0 and enter clean up
        (ins 'q2 'b 'q3 '1 'L) ; write 1 and enter clean up

        (ins 'q3 '0 'q3 'b 'L) ; clean up phase, change 0s to blanks
        (ins 'q3 '1 'q3 'b 'L) ; change 1s to blanks
        (ins 'q3 'b 'q4 'b 'R) ; when we reach the first blank on the left, we have finished cleaning and may move the head back to the output

        (ins 'q4 'b 'q4 'b 'R) ; move right over blanks
        (ins 'q4 '0 'q5 '0 'R) ; halt state, move right over output 0
        (ins 'q4 '1 'q5 '1 'R) ; halt state, move right over output 1
    ))
...
; Overview: This machine sorts a binary string by moving all 0s to the left of all 1s. The algorithm 
; repeatedly scans the string to find the first 0 that appears after a 1. Once this out-of-place 0 is 
; found, the machine effectively "picks it up" (by temporarily changing it to a 1), moves leftward 
; back to the boundary between the already-sorted 0s and the 1s, and "drops" the 0 into its correct 
; position. This process repeats until no 0s are found after 1s, indicating the string is sorted. 
; Finally, the machine moves the head to the start of the sorted string and halts.

; state 1: move over 0s until hit a 1 or empty
; state 2: move over 1s until hit a 0 or empty. once a zero is hit, switch to q3, change to 1, move left
; state 3; move left until hit a 0 or blank. once one of those is hit, move right and switch to q4
; state 4; write a 0 and move right and switch to q2
; state 5: clean up phase, move left over 0s and 1s until hit a blank, then move right and halt

(define tm-sort
    (list 
        (ins 'q1 0 'q1 0 'R) ; move right over 0s
        (ins 'q1 1 'q2 1 'R) ; switch to q2 when a 1 is read, continue moving right. everything to the left of the head is sorted
        (ins 'q1 'b 'q5 'b 'L) ; if we hit a blank, we are done sorting and can enter clean up phase

        (ins 'q2 1 'q2 1 'R) ; move right over 1s
        (ins 'q2 0 'q3 1 'L) ; when we hit a 0, change it to a 1, move left and enter q3. we have started a swap
        (ins 'q2 'b 'q5 'b 'L) ; if we hit a blank, we are done sorting and can enter clean up phase
        
        (ins 'q3 1 'q3 1 'L) ; move left over 1s
        (ins 'q3 0 'q4 0 'R) ; when we hit a 0, move right and enter q4
        (ins 'q3 'b 'q4 'b 'R) ; when we hit a blank, move right and enter q4

        (ins 'q4 1 'q2 0 'R) ; change 1 to 0, move right and enter q2 to look for next 0 to swap

        (ins 'q5 0 'q5 0 'L) ; clean up phase, move left over 0s
        (ins 'q5 1 'q5 1 'L) ; move left over 1s
        (ins 'q5 'b 'q6 'b 'R) ; when we reach the first blank on the left, we have finished sorting and may halt
    ))
