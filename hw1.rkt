...
(define (dec2bin n)
  (define result (if (<= n 1) '() (dec2bin (quotient n 2))))
  (set! result (append result (list (modulo n 2))))
  result)
...
(define (hex2dec num)
  (define digit (car num))
  (set! digit (if (number? digit)
                  digit
                  (cadr (assoc (string-upcase (symbol->string digit)) (list (list "A" 10) (list "B" 11) (list "C" 12) (list "D" 13) (list "E" 14) (list "F" 15))))))

                  ;(+
                   ;(-
                    ;(char->integer
                     ;(string-ref
                      ;(string-upcase (symbol->string digit)) 0))
                    ;(char->integer #\A))
                   ;10))) ; This could be hard-coded with just 55, but we'll do "- A + 10" for flexibility
  (+ (* digit (expt 16 (- (length num) 1))) (if (= (length num) 1)
                                                0
                                                (hex2dec (cdr num)))))

(define (dec2hex n)
  (define result (if (<= n 15) '() (dec2hex (quotient n 16))))
  (define remainder (modulo n 16))
  (set! result (append result (list (if (< remainder 10)
                                    remainder
                                    (cadr (assoc remainder (list (list 10 'A) (list 11 'B) (list 12 'C) (list 13 'D) (list 14 'E) (list 15 'F))))))))
  result)
...
(define (myassq value alist)
  (let ((head (car alist)))
  (if (eq? value (car head))
      head
      (if (= (length alist) 1)
          '()
          (myassq value (cdr alist))))))

;(define (myassq value alist) "test")
...
(define (iterate start proc n)
  (define result '())
  (set! result (append result (list (proc start))))
  (set! result (if (= n 1)
                   result
                   (append result (iterate (car result) proc (- n 1)))))
  result)
...
(define (compound start proc test)
  (define result '())
  (set! result (append result (list (proc start))))
  (set! result (if (test (car result))
                   result
                   (append result (compound (car result) proc test))))
  result)
...
(define (power-set lst)
  (define result '())
  (set! result (append result (if (= (length lst) 0)
                                  '(())
                                  (power-set (cdr lst)))))
  (define head (if (= (length lst) 0) '() (list (car lst))))
  (define temp (append '() result))
  (for-each (lambda (x) (set! result (append result (if (= (length lst) 0) '() (list (append x head)))))) temp)
  result)
...
(define (all-factors n)
  (let ((p-factors (prime-factors n))) (sort (remove-duplicates (map (lambda (x) (if (= (length x) 0) 1 (apply * x))) (power-set p-factors))) < )))
