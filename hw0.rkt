...
(define (kg2lb mass)
  (define raw (* mass 2.2))
  (define pounds (inexact->exact (floor raw)))
  (define ounces (inexact->exact (floor (* 16 (- raw pounds)))))
  (list pounds (if (= pounds 1) 'pound 'pounds) ounces (if (= pounds 1) 'ounce 'ounces))
)
